			...
			for (Users user : users) {
				...
				for (int event_id : myevents) {
					...
					for (Sessions sess : sessions) {
						...
						// "+sess.getSession_st().getTime());
						if (sess.getSession_st().getTime() - (20 * 60 * 1000) == System.currentTimeMillis()) {

							ArrayList<String> ksids = user.getKSID();
							new ScheduledTasks();
							ScheduledTasks.triggerPush(ksids,
									"Session " + sess.getSession_name() + " is going to start in couple of minutes.");
						}
					}
				}
			}
